...
const plotRight = computed(() => chartWidth.value - 18)
let resizeObserver
onMounted(() => {
  resizeObserver = new ResizeObserver(([entry]) => { chartWidth.value = Math.max(240, entry.contentRect.width) })
...
const x = date => {
  ...
  return first === last ? (62 + plotRight.value) / 2 : 62 + (timestamp(date) - first) / (last - first) * (plotRight.value - 62)
}
...
<template>
  <figure ref="chartElement" class="trend-chart">
    <figcaption>{{ title }} <small>單位：{{ unit }}</small></figcaption>
    <div v-if="!valid.length" class="chart-empty">尚無紀錄，儲存第一筆後即可查看趨勢。</div>
    <template v-else>
      <svg :viewBox="`0 0 ${chartWidth} 254`" role="img" :aria-label="title">
        <title>{{ title }}，詳細數據可展開下方表格查看</title>
        <g v-for="tick in ticks" :key="tick">
          <line x1="62" :x2="plotRight" :y1="y(tick)" :y2="y(tick)" stroke="#e0e8e2" />
        ...
        <circle v-for="point in valid" :key="point.date" :cx="x(point.date)" :cy="y(point.value)" r="7" fill="#39835b" tabindex="0" :aria-label="label(point)" @focus="selected = label(point)" @mouseenter="selected = label(point)" @click="selected = label(point)"><title>{{ label(point) }}</title></circle>
        <text x="62" y="239">{{ points[0].date.slice(5) }}</text>
        <text :x="plotRight" y="239" text-anchor="end">{{ points.at(-1).date.slice(5) }}</text>
      </svg>
...
svg { display: block; width: 100%; height: 254px; margin-top: 12px; }
svg text { font-size: 13px; fill: #617569; }
